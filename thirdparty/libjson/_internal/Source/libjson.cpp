...
#include "../../libjson.h"
    ...
    */
    ...
    void libjson_delete(JSONNODE * node){
	...
	*/
	...
	JSONNODE * libjson_copy(json_const JSONNODE * orig){
		JSON_ASSERT_SAFE(orig, JSON_TEXT("null orig to libjson_copy"), return 0;);
		#ifdef JSON_MEMORY_POOL
    ...
    //comparison
    json_bool_t libjson_equal(JSONNODE * node, JSONNODE * node2){
	   JSON_ASSERT_SAFE(node, JSON_TEXT("null node to libjson_equal"), return false;);
	   JSON_ASSERT_SAFE(node2, JSON_TEXT("null node2 to libjson_equal"), return false;);
